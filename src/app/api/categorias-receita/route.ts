...
export async function GET(request: NextRequest) {
  try {
    ...
    if (ativo === '1') {
      where += ' AND cr.ativo = 1';
    } else if (ativo === '0') {
      where += ' AND cr.ativo = 0';
    }

    const countRows = await query<{ total: number }[]>(
      `SELECT COUNT(*) AS total FROM categorias_receita cr ${where}`,
      params
    );
    ...
    const list = await query<
      { id: number; plano_contas_id: number; nome: string; descricao: string | null; ativo: number; plano_nome: string }[]
    >(
      `SELECT cr.id, cr.plano_contas_id, cr.nome, cr.descricao, cr.ativo,
              CONCAT(pc.codigo, ' - ', pc.nome) AS plano_nome
       FROM categorias_receita cr
       INNER JOIN plano_contas pc ON pc.id = cr.plano_contas_id
       ${where} ORDER BY cr.nome LIMIT ${limit} OFFSET ${offsetSafe}`,
      params
    );
    ...
    console.error('API categorias-receita GET:', e);
...
export async function POST(request: NextRequest) {
  try {
    ...
    await query(
      `INSERT INTO categorias_receita (plano_contas_id, nome, descricao, ativo)
       VALUES (?, ?, ?, ?)`,
      [
    ...
    console.error('API categorias-receita POST:', e);
    const message = e instanceof Error ? e.message : String(e);
    return NextResponse.json(
      {
        success: false,
        error: 'Erro ao cadastrar categoria de receita.',
        ...(process.env.NODE_ENV !== 'production' && { detail: message }),
      },
      { status: 500 }
    );
