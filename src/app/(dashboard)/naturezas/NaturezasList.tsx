...
type Item = { id: number; codigo: string | null; nome: string; tipo: string; ordem: number; ativo: number };

export function NaturezasList() {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 flex gap-2 flex-wrap items-center justify-between">
        ...
        <Link href="/naturezas/novo" className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Nova natureza</Link>
      </div>
      {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={ativoFilter === 'all' ? 6 : 5} className="p-6 text-center text-slate-500">Nenhuma natureza encontrada.</td>
                  </tr>
                ) : (
                  items.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="p-3 text-slate-600">{r.codigo ?? '—'}</td>
                      <td className="p-3 font-medium text-slate-800">{r.nome}</td>
                      <td className="p-3 text-slate-600 capitalize">{r.tipo}</td>
                      <td className="p-3 text-slate-600">{r.ordem}</td>
                      {ativoFilter === 'all' && (
                        <td className="p-3">
                      ...
                      <CrudActionsCell basePath="/naturezas" id={r.id} entityLabel={r.nome} onDeleted={refetch} />
                    </tr>
                  ))
                )}
              </tbody>
