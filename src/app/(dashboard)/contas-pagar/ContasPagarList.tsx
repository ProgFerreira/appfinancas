...
type Options = { contasBancarias: { id: number; descricao: string }[]; categoriasDespesa: { id: number; nome: string }[] };

export function ContasPagarList() {
  const router = useRouter();
  const [items, setItems] = useState<(ContaPagar & { fornecedor_nome?: string; plano_contas_nome?: string; categoria_nome?: string; centro_custo_nome?: string; conta_bancaria_descricao?: string })[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/contas-pagar?${q}`)
      .then(async (res) => {
        const text = await res.text();
        if (!text) return { success: false, error: 'Resposta vazia', status: res.status };
        try {
          return { ...JSON.parse(text), status: res.status } as { success: boolean; data?: (ContaPagar & { fornecedor_nome?: string; plano_contas_nome?: string; categoria_nome?: string; centro_custo_nome?: string; conta_bancaria_descricao?: string })[]; meta?: { total: number; totalPages: number }; error?: string; detail?: string; status: number };
        } catch {
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <input
              type="date"
              value={dataVencDe}
              onChange={(e) => { setDataVencDe(e.target.value); setPage(1); }}
              className="px-3 py-2 rounded-lg border border-slate-300 text-sm max-w-[140px]"
              placeholder="Venc. de"
              title="Vencimento de"
            />
            <input
              type="date"
              value={dataVencAte}
              onChange={(e) => { setDataVencAte(e.target.value); setPage(1); }}
              className="px-3 py-2 rounded-lg border border-slate-300 text-sm max-w-[140px]"
            ...
            <select
              ...
              onChange={(e) => { setContaBancariaId(e.target.value); setPage(1); }}
              className="px-3 py-2 rounded-lg border border-slate-300 text-sm min-w-[160px]"
            >
            ...
            />
            <button type="button" onClick={applyFilters} className="btn btn-secondary btn-sm">Filtrar</button>
          </div>
          <Link href="/contas-pagar/novo" className="btn btn-primary">Nova conta a pagar</Link>
        </div>
      </div>
      {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="text-left p-3">Forma pag.</th>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="p-3 font-medium text-slate-800">{r.fornecedor_nome ?? '—'}</td>
