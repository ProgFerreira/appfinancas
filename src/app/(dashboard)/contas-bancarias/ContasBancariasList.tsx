            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="text-left p-3">Agência / Conta</th>
                ...
                {items.length === 0 ? (
                  ...
                  items.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="p-3 text-slate-600">{r.banco}</td>
                      <td className="p-3 text-slate-600">
                        {[r.agencia, r.conta].filter(Boolean).join(' / ') || '—'}
                      </td>
                      <td className="p-3 text-slate-600 capitalize">{r.tipo}</td>
                      <td className="p-3 text-right font-medium text-slate-800">
                        {formatMoney(r.saldo_inicial)}
                      </td>
                      {ativoFilter === 'all' && (
                        <td className="p-3">
                          <span
