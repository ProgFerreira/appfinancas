...
export default function BaixaContaPagarPage() {
  ...
  useEffect(() => {
    if (conta && !form.valor_pago) setForm((f) => ({ ...f, valor_pago: String(conta.valor) }));
  }, [conta]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      valor_pago: Number(form.valor_pago.replace(',', '.')),
      conta_bancaria_id: form.conta_bancaria_id || null,
      observacoes: form.observacoes || null,
    };
    try {
      const res = await fetch(`/api/contas-pagar/${id}/baixa`, {
        method: 'POST',
  ...
  if (loading || !conta) {
    return (
      <>
        <Breadcrumb items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Contas a pagar', href: '/contas-pagar' }, { label: 'Registrar pagamento' }]} />
  ...
  return (
    <>
      ...
      <div className="mt-4 form-panel">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">Registrar pagamento</h2>
        <div className="mb-4 p-4 rounded-lg bg-slate-50 text-sm">
          <p><strong>Fornecedor:</strong> {conta.fornecedor_nome ?? '—'}</p>
        ...
        <div className="form-panel rounded-xl border border-slate-200 bg-white shadow-sm p-6 form-contraste">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Link href="/contas-pagar" className="px-4 py-2 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50">
                Cancelar
              </Link>
            </div>
          </form>
