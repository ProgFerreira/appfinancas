...
type Item = { id: number; nome: string; cpf: string; tipo_vinculo: string; parceiro_nome: string | null; ativo: number; veiculos_count?: number };
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 flex gap-2 flex-wrap justify-between items-center">
        ...
        <Link href="/motoristas/novo" className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Novo motorista</Link>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="text-left p-3">Tipo vínculo</th>
                  <th className="text-left p-3">Parceiro</th>
                  <th className="text-left p-3">Veículos</th>
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={ativoFilter === 'all' ? 7 : 6} className="p-6 text-center text-slate-500">Nenhum motorista.</td></tr>
                ) : items.map((r) => (
                  <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="p-3 font-medium text-slate-800">{r.nome}</td>
                    <td className="p-3 text-slate-600">{r.cpf}</td>
                    <td className="p-3 text-slate-600">{r.tipo_vinculo}</td>
                    <td className="p-3 text-slate-600">{r.parceiro_nome ?? '—'}</td>
                    <td className="p-3 text-slate-600">
                      <Link href={`/motoristas/${r.id}/visualizar`} className="text-indigo-600 hover:underline">
                        {r.veiculos_count ?? 0} {Number(r.veiculos_count) === 1 ? 'veículo' : 'veículos'}
                      </Link>
                    </td>
                    ...
                    <CrudActionsCell basePath="/motoristas" id={r.id} entityLabel={r.nome} onDeleted={refetch} />
                  </tr>
                ))}
              </tbody>
            </table>
