...
export async function GET(
  ...
  try {
    ...
    >(
      'SELECT id, cte_id, cnpj, inscricao_estadual, nome, telefone, logradouro, numero, complemento, bairro, codigo_municipio, municipio, cep, uf, codigo_pais, pais FROM cte_destinatarios WHERE cte_id = ? ORDER BY id',
      [cteId]
    );
    return NextResponse.json({ success: true, data: Array.isArray(rows) ? rows : [] });
  } catch (e) {
    console.error('API ctes destinatarios GET:', e);
    return NextResponse.json({ success: false, error: 'Erro ao listar destinatários.' }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    await query(
      `INSERT INTO cte_destinatarios (cte_id, cnpj, inscricao_estadual, nome, telefone, logradouro, numero, complemento, bairro, codigo_municipio, municipio, cep, uf, codigo_pais, pais)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        cteId,
        toStr(body.cnpj),
    ...
    console.error('API ctes destinatarios POST:', e);
    return NextResponse.json({ success: false, error: 'Erro ao cadastrar destinatário.' }, { status: 500 });
  }
}
