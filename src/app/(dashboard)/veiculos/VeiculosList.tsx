...
type Item = { id: number; placa: string; modelo: string; tipo: string; ano: number | null; ativo: number };
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 flex gap-2 flex-wrap justify-between items-center">
        ...
        <Link href="/veiculos/novo" className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Novo veículo</Link>
      </div>
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={ativoFilter === 'all' ? 6 : 5} className="p-6 text-center text-slate-500">Nenhum veículo.</td></tr>
                ) : items.map((r) => (
                  <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="p-3 font-medium text-slate-800">{r.placa}</td>
                    <td className="p-3 text-slate-600">{r.modelo}</td>
                    <td className="p-3 text-slate-600">{r.tipo}</td>
                    <td className="p-3 text-slate-600">{r.ano ?? '—'}</td>
                    ...
                    <CrudActionsCell basePath="/veiculos" id={r.id} entityLabel={r.placa} onDeleted={refetch} />
                  </tr>
                ))}
              </tbody>
            </table>
