...
export function UsuariosList() {
  ...
  useEffect(() => {
    ...
    if (statusFilter === 'inativo') q.set('status', 'inativo');
    else if (statusFilter === 'ativo') q.set('status', 'ativo');
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 flex gap-2 flex-wrap items-center justify-between">
        ...
        <Link href="/usuarios/novo" className="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Novo usuário</Link>
      </div>
      {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  {statusFilter === 'all' && <th className="text-left p-3">Ativo</th>}
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={statusFilter === 'all' ? 6 : 5} className="p-6 text-center text-slate-500">Nenhum usuário encontrado.</td>
                  </tr>
                ) : (
                  items.map((r) => (
                    <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="p-3 text-slate-600">{perfilLabel[r.perfil] ?? r.perfil}</td>
                      <td className="p-3">
                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${r.status === 'ativo' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'}`}>{r.status}</span>
                      </td>
                      {statusFilter === 'all' && (
                        <td className="p-3">
                      ...
                      <CrudActionsCell basePath="/usuarios" id={r.id} entityLabel={r.nome} onDeleted={refetch} />
                    </tr>
                  ))
                )}
              </tbody>
