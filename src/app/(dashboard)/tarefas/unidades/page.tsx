...
export default function TiposUnidadePage() {
  const [items, setItems] = useState<TarefaUnidade[]>([]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setMessage(null);
    const url = editingId ? `/api/tarefa-unidades/${editingId}` : '/api/tarefa-unidades';
  ...
  function startEdit(u: TarefaUnidade) {
    setEditingId(u.id);
    setForm({ nome: u.nome, ordem: u.ordem, ativo: u.ativo });
  }
  ...
  function handleDelete(id: number, nome: string) {
    if (!confirm(`Excluir o tipo "${nome}"? As tarefas que usam este tipo ficarão sem unidade.`)) return;
    fetch(`/api/tarefa-unidades/${id}`, { method: 'DELETE' })
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  return (
    <>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-semibold text-slate-800">Tipos de unidade</h2>
        <Link
          href="/tarefas"
      ...
      <p className="mt-1 text-sm text-slate-600">
        Cadastre os tipos de unidade (ex.: Empresa, Pessoal, Ministério, Família) para classificar tarefas.
      </p>
              ...
              {items.map((u) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <span className="ml-2 text-xs text-slate-500">ordem: {u.ordem}</span>
                    {u.ativo === 0 && (
